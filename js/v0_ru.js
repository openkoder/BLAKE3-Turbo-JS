...
function readLittleEndianWords(array, offset, words) {
  ...
  // Читаем полные 4-байтовые слова
  for (; offset + 3 < array.length; ++i, offset += 4) {
    words[i] =
      array[offset] |                  // байт 0 → биты 0-7
      (array[offset + 1] << 8) |       // байт 1 → биты 8-15
      (array[offset + 2] << 16) |      // байт 2 → биты 16-23
      (array[offset + 3] << 24);       // байт 3 → биты 24-31
  }
  
  // Заполняем оставшиеся слова нулями (паддинг)
  for (let j = i; j < words.length; ++j) {
    words[j] = 0;
  }
  
  // Обрабатываем оставшиеся 1-3 байта (неполное слово)
...
function blake3(input) {
  ...
  // Читаем последний (возможно неполный) блок
  readLittleEndianWords(input, offset, blockWords);
