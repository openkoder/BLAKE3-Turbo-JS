/**
 * ============================================================================
 * BLAKE3 — Naive JavaScript Implementation (Working v6)
 * ============================================================================
 * 
...
/**
 * ============================================================================
 * BLAKE3 v6 — Message Block in Local Variables (SMI)
 * ============================================================================
 * 
 ...
 * - v6: Message block in local variables m_0...m_15
 *       Removal of PERMUTATIONS table (112 bytes)
 *       Physical permutation via two cycles (only 2 temporary variables)
...
function compress(
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // Message initialization m_ - 16 local variables instead of Uint32Array
...
function blake3(input) {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // Pre-allocated buffers — NO ALLOCATIONS IN THE HOT LOOP
  // ═══════════════════════════════════════════════════════════════════════
  
  // Maximum tree depth: log2(max_chunks) = 54 is enough for 2^54 chunks
  const maxDepth = 54;
  const cvStack = new Uint32Array(maxDepth * 8);  // Stack for Merkle tree. 8 words per level
  let cvStackPos = 0;

  // Buffer for current CV (8 words) and message block (16 words)
  // Combined into one array for better locality
  const workBuffer = new Uint32Array(8 + 16);  // cv[0..7] + block[0..15]
  const CV_OFFSET = 0;
