...
/**
 * ============================================================================
 * BLAKE3 v6 — Блок сообщения в локальных переменных (регистры процессора)
 * ============================================================================
 * 
 ...
 * - v5: Zero-copy — in-place запись, стек на Uint32Array
 * - v5: Zero-copy — in-place запись в compress() в out[outOffset...], cvStack как Uint32Array
 *       Удаление таблицы PERMUTATIONS (112 байт)
 *       Физическая перестановка через два цикла (только 2 временные переменные)
...
function compress(
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // Инициализация сообщения m_ - 16 локальных переменных вместо Uint32Array
...
function blake3(input) {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // Предаллоцированные буферы — НИКАКИХ АЛЛОКАЦИЙ В ГОРЯЧЕМ ЦИКЛЕ
  // ═══════════════════════════════════════════════════════════════════════
  
  // Максимальная глубина дерева: log2(max_chunks) = 54 достаточно для 2^54 чанков
  const maxDepth = 54;
  const cvStack = new Uint32Array(maxDepth * 8);  // Стек для дерева Меркла. 8 слов на уровень
  let cvStackPos = 0;
