...
// Based on the Fleek Network case study
...
// Blake3 is little-endian friendly
...
function getCvStack(maxDepth: number): Uint32Array {
...
// Optimized compress function using local variables
function compress(
  cv: Uint32Array,
  cvOffset: number,
  ...
  // Load state into local variables for better JIT optimization
  ...
  // Load message words into local variables
  ...
  for (let i = 0; i < 7; ++i) {
    ...
    // Permute message words (skip on last round)
    if (i != 6) {
      const t0 = m_0;
      const t1 = m_1;
      m_0 = m_2;
  ...
  // Write output (write 8..15 first in case out == cv)
  if (!truncateOutput) {
...
export function hash(input: Uint8Array): Uint8Array {
  const inputWords = new Uint32Array(
    input.buffer as ArrayBuffer, // Error! ← добавить as ArrayBuffer
    input.byteOffset,
    input.byteLength >> 2,
  );
  ...
  // Process full chunks (1024 bytes each)
